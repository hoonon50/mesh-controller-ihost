...
if not APP.exists():
    raise SystemExit("v6.3.0: app.py nenalezen")
if not INDEX.exists():
    raise SystemExit("v6.3.0: templates/index.html nenalezen")

app = APP.read_text(encoding="utf-8")
marker = "# v6.3.0 LAN port device inspector"
if marker not in app:
    m = re.search(r"(?m)^(\s*app\s*=\s*Flask\([^\n]*\)\s*)$", app)
    if not m:
        raise SystemExit("v6.3.0: app = Flask(...) nenalezen")
    injection = (
        m.group(1) + "\n\n" + marker + "\n"
...
html = re.sub(
    r'<script\s+[^>]*src=["\']/static/v630_lan_port_inspector\.js(?:\?v=[^"\']+)?["\'][^>]*></script>',
