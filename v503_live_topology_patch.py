...
if not APP.exists():
    raise SystemExit("v5.0.7: app.py nenalezen")
if not INDEX.exists():
    raise SystemExit("v5.0.7: templates/index.html nenalezen")
...
if marker not in app:
    ...
    if not m:
        raise SystemExit("v5.0.7: nepodařilo se najít app = Flask(...)")
    injection = (
        m.group(1) + "\n\n" + marker + "\n"
...
if css_tag not in html:
    if "</head>" not in html.lower():
        raise SystemExit("v5.0.7: index.html nemá </head>")
    html = re.sub(r"</head>", f"  {css_tag}\n</head>", html, count=1, flags=re.I)
if js_tag not in html:
    if "</body>" not in html.lower():
        raise SystemExit("v5.0.7: index.html nemá </body>")
...
print("v5.0.7: explicitní /api/v503/live-topology + vlastní živý topology renderer")
print("v5.0.7: starý v5.0.2 timer interception odstraněn")
